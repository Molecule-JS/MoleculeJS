...
for (const format in rollupBuilds) {
  ...
  gulp.task(`rollup:dist:${format}`, () => runSequence(...sources.map(src => `rollup:dist:${format}:${src}`)));
}

for (const src of sources)
  gulp.task(`rollup:dev:${src}`, () => runSequence(...Object.keys(rollupBuilds).map(format => `rollup:dev:${format}:${src}`)));
...
for (const format in rollupBuilds) {
  ...
  gulp.task(`rollup:dev:${format}`, () => runSequence(...sources.map(src => `rollup:dev:${format}:${src}`)));
}
