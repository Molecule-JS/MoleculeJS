...
const { expect } = chai;
export const propTests = (testElement) => describe('properties', () => {
    it('correct type', () => {
    ...
    it('calls observer', () => {
        expect(window.observerVals.get('bool')).to.equal(testElement.longBool);
        expect(window.observerVals.get('number')).to.equal(testElement.longNumber);
    });
});
