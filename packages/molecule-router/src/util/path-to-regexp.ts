/*
  Ideally we would just import path-to-regexp, but because their ts declerations are flawed,
  we have to port it to TS manually.
*/
 ...
 */
function escapeString(str: string | null) {
  if (!str) {
    return str;
  }
 ...
 */
function stringToRegexp(path: string, keys?: Key[], options?: RegExpOptions) {
...
export function tokensToRegExp(
  ...
  const endsWith = [...((options.endsWith as string[]) || [])]
    .map(escapeString)
    .concat('$')
    .join('|');
  let route = '';
